...
interface Props { result: ExecutionRecord; onClose: () => void; onRetry: () => void; }

export default function ExecutionResult({ result, onClose, onRetry }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap justify-end gap-2"><Button variant="outline" onClick={onRetry}>{t("execution.retry")}</Button><Button onClick={onClose}>{t("execution.close")}</Button></div>
    </div>
  );
}
